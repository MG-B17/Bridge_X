...
- **Email Verification** — 6-digit OTP verification after registration, resend code with 30s cooldown timer (starts automatically on screen open)
...
### Session Persistence — Incomplete Setup

On app restart, `AppInitializer` checks whether the stored session has a complete profile. If the user's profile is incomplete (`isProfileComplete == false` or `userName.isEmpty`), the stored token and user data are deleted and the user is forced to the login screen. This prevents users from being dumped back into an unfinished setup flow after closing the app.
...
>
> The `isProfileComplete` field is parsed from either `is_profile_complete` or `profile_completed` (legacy key) in the JSON response.
>
...
> - `is_profile_complete` — checked on app restart to decide whether to preserve the session
...
VerificationCubit created
    │
VerificationState(status: initial, cooldownSeconds: 0)
    │
    ├── startResendCooldown()      ← called automatically on screen open
    │       └─► emits cooldownSeconds: 30, starts 30s countdown timer
    │
    ├── verifyEmail(email, code) → loading → (success | error)
    │
    └── resendVerify(email)       ← guarded by state.canResend (cooldownSeconds <= 0)
            │
            ├── success → emits cooldownSeconds: 30, restarts countdown
            └── failure → emits status: error, message
```

| Guard | Behavior |
|---|---|
| `cooldownSeconds > 0` | `resendVerify()` returns early — button disabled in UI |
| `status == loading` | Button text shows loading state in UI |
...
| **State** | `VerificationState(status, message, cooldownSeconds)` + `canResend` getter (`cooldownSeconds <= 0`) |
| **Actions** | `verifyEmail(email, code)`, `resendVerify(email)`, `startResendCooldown()` |
| **Timer** | 30-second countdown via `Timer.periodic`; starts immediately on screen open via `startResendCooldown()` in `BlocProvider.create`; also restarts on successful `resendVerify()` |
| **Cooldown guard** | `resendVerify()` returns early if `!state.canResend` — prevents duplicate API calls during cooldown |
...
| Aspect | Detail |
|---|---|
| **Dependencies** | `ChangePasswordUsecase`, `LogoutUseCase`, `SoftDeleteProfileUseCase`, `AppState`, `PushNotificationService` |
| **State** | `AccountState(status, message, action)` — uses `AuthAction` enum for action tracking |
| **Actions** | `changePassword()`, `logout()`, `softDeleteProfile()`, `completeSoftDeleteSignOut()` |
| **Soft delete flow** | API call → emit success → show confirmation dialog → on OK call `completeSoftDeleteSignOut()` which cleans local data via `LogoutUseCase`, resets `AppState` (`isLoggedIn=false`, `userData=null`), and refreshes push token |
| **File** | `presentation/controller/account/account_cubit.dart` |
...
- `AccountCubit.softDeleteProfile()` calls `SoftDeleteProfileUseCase` → `POST /api/profile/soft-delete`
- On API success: emits `status: success, action: softDeleteProfile`
- Screen shows a `SuccessDialog`; when user taps OK, `AccountCubit.completeSoftDeleteSignOut()` is called
- `completeSoftDeleteSignOut()`: calls `LogoutUseCase` to clean local data (token, cache), resets `AppState` (`isLoggedIn=false`), and refreshes push token
- Navigation guard detects `isLoggedIn=false` and redirects to login
...
- `VerificationCubit.startResendCooldown()` is called automatically when the verify screen opens (in `BlocProvider.create`)
- Emits `cooldownSeconds: 30` immediately, then a `Timer.periodic` counts down every second
- During cooldown (`cooldownSeconds > 0`): `canResend` is `false`, `resendVerify()` returns early, the resend button is disabled and shows "Resend code in Xs"
- After 30s (`cooldownSeconds == 0`): button re-enables, showing "Resend code" text
- On successful resend: cooldown restarts from 30s; any previous error message is cleared
- Does NOT emit `loading` or `success` states — only `cooldownSeconds` is updated to avoid triggering navigation listeners

### 🔄 App Restart — Incomplete Profile Clears Session
- On app restart, `AppInitializer.init()` reads stored token and `userData` from `SecureStorage`
- If `isLoggedIn == true` but `userData.isProfileComplete == false` or `userData.userName.isEmpty`, the session is considered incomplete
- In that case: token + userData are deleted from `SecureStorage`, cache is cleared, and `AppInitializerResult(isLoggedIn: false)` is returned
- This forces the navigation guard to redirect to the login screen instead of resuming an unfinished setup flow
...
| `verification_cubit_test.dart` | 6 | verifyEmail success + error + entity mapping, resendVerify success + error + cooldownSeconds set + entity mapping |

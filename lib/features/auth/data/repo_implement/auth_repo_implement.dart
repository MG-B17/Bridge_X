...
class AuthRepoImplement extends AuthRepo {
  ...
  Future<Either<Failure, String>> forgetPassword({
    ...
    if (await networkInfo.isConnected) {
      try {
        LoggerService.debug(
          'Requesting password reset for: ${forgetPasswordEntity.email}',
          tag: 'AuthRepo',
        );
  ...
  Future<Either<Failure, LoginResultEntity>> login({
    ...
    if (await networkInfo.isConnected) {
      try {
        LoggerService.debug(
          'Attempting login for: ${loginEntity.email}',
          tag: 'AuthRepo',
        );
        final response = await authRemoteData.login(loginEntity: loginEntity);
        final String userEncodedData = UserDataModel.userEncodedata(
        ...
        try {
          await Future.wait([
            ...
            secureStorageService.write(key: AppKeys.userDataKey, value: userEncodedData),
          ]);
        } catch (e) {
          LoggerService.error(
            'Failed to persist login data',
            exception: e,
            tag: 'AuthRepo',
          );
        }
        LoggerService.info(
          'Login successful for: ${loginEntity.email}',
          tag: 'AuthRepo',
        );
        return Right(response.toEntity());
      } on ServerException catch (e) {
        LoggerService.error(
          'Login failed for: ${loginEntity.email}',
          exception: e,
          tag: 'AuthRepo',
        );
        return Left(ServerFailure(message: e.message!));
  ...
  Future<Either<Failure, String>> register({
    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        final result = await authRemoteData.register(
          registerEntity: registerEntity,
        );
        LoggerService.info(
          'Registration successful for: ${registerEntity.email}',
          tag: 'AuthRepo',
        );
        return Right(result);
      } on ServerException catch (e) {
        LoggerService.error(
          'Registration failed for: ${registerEntity.email}',
          exception: e,
          tag: 'AuthRepo',
        );
        return Left(ServerFailure(message: e.message!));
  ...
  Future<Either<Failure, String>> resendVerify({required String email}) async {
    if (await networkInfo.isConnected) {
      try {
        ...
        LoggerService.error(
          'Resend verify failed for: $email',
          exception: e,
          tag: 'AuthRepo',
        );
        return Left(ServerFailure(message: e.message!));
  ...
  Future<Either<Failure, String>> resetPassword({
    ...
    if (await networkInfo.isConnected) {
      try {
        final result = await authRemoteData.resetPassword(
  ...
  Future<Either<Failure, String>> verifyEmail({
    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        final result = await authRemoteData.verifyEmail(
  ...
  Future<Either<Failure, void>> logout() async {
    if (await networkInfo.isConnected) {
      try {
        ...
        await cacheService.clearData();
        LoggerService.error(
          'Logout API failed, local token still deleted',
          exception: e,
          tag: 'AuthRepo',
        );
        return Left(ServerFailure(message: e.message!));
      ...
      LoggerService.warning(
        'No internet — local token deleted anyway',
        tag: 'AuthRepo',
      );
      return Right(null);
    }
  ...
  Future<Either<Failure, Unit>> completeProfile({
    ...
    if (await networkInfo.isConnected) {
      try {
        LoggerService.debug(
          'Completing profile: track=$track, level=$experienceLevel',
          tag: 'AuthRepo',
        );

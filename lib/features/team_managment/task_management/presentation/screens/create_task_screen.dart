...
import 'package:bridge_x/features/team_managment/task_management/presentation/widgets/create_task_widgets/assign_to_section.dart';
import 'package:bridge_x/features/team_managment/task_management/presentation/widgets/create_task_widgets/create_task_form_fields.dart';
import 'package:bridge_x/features/team_managment/task_management/presentation/widgets/create_task_widgets/create_task_header.dart';
import 'package:bridge_x/features/team_managment/task_management/presentation/widgets/create_task_widgets/tags_section.dart';
...
class _CreateTaskScreenState extends State<CreateTaskScreen> {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        BlocSelector<
                          ...
                          DateTime?
                        >(
                          selector: (state) => state is CreateTaskReady
                              ? state.selectedDate
                              : null,
                          builder: (context, date) => DueDatePriorityRow(
                            selectedDate: date,
  ...
  void _pickDate() async {
    ...
    if (picked != null && mounted) {
      context.read<CreateTaskCubit>().setDate(picked);
    }
  }
  ...
  void _submit() {
    ...
    if (state is! CreateTaskReady) return;
    if (state.selectedMemberId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text(TaskStrings.selectMemberFirst)),
        );
      }
      return;
    }
    if (state.selectedDate == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text(TaskStrings.selectDateFirst)),
        );
      }
      return;
    }
    ...
    cubit.submitTask(
      ...
      gitLink: _gitLinkController.text.trim().isNotEmpty
          ? _gitLinkController.text.trim()
          : null,
    );
  }
  ...
  void _handleStateChange(BuildContext context, CreateTaskState state) {
    if (state is CreateTaskLoading) {
      LoadingDialog.show(context: context, message: TaskStrings.creatingTask);
    } else if (state is CreateTaskSuccess) {
      LoadingDialog.hide(context);
      SuccessDialog.show(
        context: context,
        title: TaskStrings.taskCreated,
        message: TaskStrings.taskCreatedMessage,

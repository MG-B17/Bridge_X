...
import 'package:bridge_x/features/team_managment/task_management/presentation/widgets/create_task_widgets/priority_selector.dart';
...
class TaskTitleField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const CreateTaskLabel(text: TaskStrings.taskTitle, isRequired: true),
        VerticalSpacing(AppSpacing.spacing8),
        BridgeXTextFormField(
          hint: TaskStrings.taskTitleHint,
          controller: controller,
          validator: (v) => AppValidator.required(v, TaskStrings.taskTitle),
        ),
      ],
    );
  }
...
class GitLinkField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const CreateTaskLabel(text: TaskStrings.githubUrl),
        VerticalSpacing(AppSpacing.spacing8),
        BridgeXTextFormField(
          hint: 'URL',
          controller: controller,
...
class TaskDescriptionField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const CreateTaskLabel(text: TaskStrings.taskDetails),
        VerticalSpacing(AppSpacing.spacing8),
        BridgeXTextFormField(
          hint: TaskStrings.taskDetailsHint,
          controller: controller,
          maxLines: 5,
          minLines: 4,
          validator: (v) => AppValidator.required(v, TaskStrings.taskDetails),
        ),
      ],
    );
  }
      ...
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const CreateTaskLabel(text: TaskStrings.dueDate),
              VerticalSpacing(AppSpacing.spacing8),
              GestureDetector(
                onTap: onPickDate,
                child: Container(
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const CreateTaskLabel(text: TaskStrings.priority),
              VerticalSpacing(AppSpacing.spacing8),
              BlocSelector<CreateTaskCubit, CreateTaskState, String>(
                selector: (state) => state is CreateTaskReady ? state.priority : 'medium',
